...
def train(saleset, data, x_set, y_set):
    ...
    return svc_parameter, svc_score,log_reg_parameter, log_reg_score,saleset

def import_model(parameter,score,saleset,aid,tablename):
    """
    导入数据模型到数据库中
    ...
    cursor = db.cursor()
    #判断数据库中是否有相同的用户ID
    sql = "select DISTINCT aid from '" + tablename +"'"
    sql = sql.replace('\'','')
    cursor.execute(sql)
    db.commit()
    aid_set = cursor.fetchall()
    if(tablename == 'regression'):
        # 如果数据库不为空
        if (len(aid_set)):
            for i in aid_set:
...
def main(aid,filepath):
    ...
    svc_parameter, svc_score, log_reg_parameter, log_reg_score, saleset = train(saleset, data, x_set, y_set)
    import_model(log_reg_parameter, log_reg_score, saleset, aid, 'regression')
    import_model(svc_parameter, svc_score, saleset, aid, 'svm')
